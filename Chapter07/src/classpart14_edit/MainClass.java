	...
	public static void main(String[] args) {
		...
		while(run) {
			...
			case 2:
				...
				// 추가
				  if(list[selectStudent].subjects == null) {
					  list[selectStudent].subjects = new Subject[3];   // 선언
				  }
				// 추가
					...
					if(subCnt1>=3) {
						...
						continue;
						  ...
						  list[selectStudent].subjects[subCnt1].score = 0;
							...
							System.out.println(list[selectStudent].subjects[subCnt1].name+"과목="+
			...
			case 3:
				...
				if(selectStudent==0) {
					for(int i=0;i<subCnt1;i++) {
						System.out.println("["+(i+1)+"]"+list[selectStudent].subjects[i].name);
					}
					System.out.print("과목 선택 : ");
					inputSubjectIndex = scan.nextInt();
					System.out.println("성적 입력 : ");
					inputScore = scan.nextInt();
					list[selectStudent].subjects[inputSubjectIndex-1].score = inputScore;
				}
